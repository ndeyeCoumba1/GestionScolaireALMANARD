...
export default function InscriptionList() {
  ...
  const totalFrais = inscriptions.reduce((sum, i) => sum + (i.fraisInscription || 0), 0);

  const classeData = inscriptions.reduce((acc: { [key: string]: number }, i) => {
    const classe = i.classeNiveau || 'Non défini';
    acc[classe] = (acc[classe] || 0) + 1;
    return acc;
  }, {});

  const classeChartData = Object.entries(classeData).map(([name, value]) => ({ name, value }));
    ...
    <div className="d-flex flex-column gap-4">
      ...
      <div className="bg-white rounded-4 shadow-sm p-4" style={{ border: '1px solid #f0f0f0' }}>
        <div className="d-flex align-items-center gap-3">
          <div className="rounded-circle d-flex align-items-center justify-content-center" style={{ width: 56, height: 56, backgroundColor: '#dbeafe', fontSize: 28 }}>
            📝
          </div>
          <div>
            <p className="text-muted mb-0" style={{ fontSize: 13, fontWeight: 500 }}>Total des Frais d'Inscription</p>
            <p className="fw-bold mb-0" style={{ fontSize: 28, color: '#1d4ed8' }}>{totalFrais.toLocaleString('fr-FR')} FCFA</p>
          </div>
        </div>
      </div>
      ...
      {/* ── Charts Section ── */}
      <div className="bg-white rounded-4 shadow-sm p-4" style={{ border: '1px solid #f0f0f0' }}>
        <p className="fw-semibold mb-4" style={{ fontSize: 15, color: '#111827' }}>Répartition des Inscriptions par Classe</p>
                  ...
                  <td className="py-3 px-4">
                    <div className="d-flex align-items-center gap-2">
                      ...
                      <button
                        ...
                        title="Supprimer"
                        className="btn btn-sm d-flex align-items-center justify-content-center"
                        style={{ width: 32, height: 32, padding: 0, borderRadius: 8, border: '1px solid #ef4444', backgroundColor: '#fef2f2', color: '#ef4444', opacity: deletingId === i.id ? 0.4 : 1 }}
