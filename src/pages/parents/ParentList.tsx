...
export default function ParentList() {
  ...
  const filtered = parents.filter(p =>
    `${p.nom} ${p.prenom} ${p.telephone} ${p.email ?? ''} ${p.profession ?? ''}`
  ...
  return (
    <div className="d-flex flex-column gap-4">
      ...
      {/* ── Header ── */}
      <PageHeader
        subtitle="Gestion des parents"
        title="Parents"
        description="Suivez, recherchez et gérez les parents adultes responsables."
      ...
      <div className="bg-white rounded-4 shadow-sm overflow-hidden" style={{ border: '1px solid #f0f0f0' }}>
        <div className="px-4 pt-4 pb-3">
          <p className="fw-semibold text-dark mb-3" style={{ fontSize: 15 }}>Liste des parents</p>
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={p.id} style={{ borderTop: '1px solid #f3f4f6' }}>
                  ...
                  <td className="py-3 px-4" style={{ color: '#6b7280' }}>{p.telephone}</td>
                  <td className="py-3 px-4" style={{ color: '#9ca3af' }}>{p.email || '—'}</td>
                  <td className="py-3 px-4">
                    {p.profession
                      ? <span className="badge rounded-pill fw-medium" style={{ backgroundColor: '#dcfce7', color: '#166534', fontSize: 12, padding: '5px 10px' }}>{p.profession}</span>
